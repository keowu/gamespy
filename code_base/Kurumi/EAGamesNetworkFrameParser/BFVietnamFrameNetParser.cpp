...
BFVietnamFrameNetParser::BFVietnamFrameNetParser(std::string& strFilePath) {
...
auto BFVietnamFrameNetParser::getRequesterIp() -> std::string {

	char chRequesterIp[64]{ 0 };

	sprintf_s(chRequesterIp, "%d.%d.%d.%d\n", this->m_dwRequesterIP & 0xff, (this->m_dwRequesterIP >> 8) & 0xff, (this->m_dwRequesterIP >> 16) & 0xff, (this->m_dwRequesterIP >> 24) & 0xff);

	return std::string(chRequesterIp);
}

auto BFVietnamFrameNetParser::setRequesterIp(const char* chIpv4) -> void {

	int b1{ 0 }, b2{ 0 }, b3{ 0 }, b4{ 0 }, port{ 0 };

	sscanf_s(chIpv4, "%d.%d.%d.%d", &b1, &b2, &b3, &b4);

	this->m_dwRequesterIP = (b4 << 24) |
		(b3 << 16) |
		(b2 << 8) |
		b1;
...
auto BFVietnamFrameNetParser::addServer(const char* chIpv4, const char* chPort) -> void {
	...
	BFVietnamgameServer bfV{ 0 };

	bfV.dwServerIP = (b4 << 24) |
		(b3 << 16) |
		(b2 << 8) |
		b1;

	bfV.wServerPort = _byteswap_ushort(port);

	this->m_BfVietnamServers.push_back(bfV);
}

auto BFVietnamFrameNetParser::deleteServer(int index) -> void {
...
auto BFVietnamFrameNetParser::getNewPayload() -> unsigned char* {
...
auto BFVietnamFrameNetParser::getRawPayload() -> unsigned char* {

	return this->m_ucRawPayload;
}

auto BFVietnamFrameNetParser::writeNewPayload(std::string& path) -> void {
...
BFVietnamFrameNetParser::operator std::string() const {
